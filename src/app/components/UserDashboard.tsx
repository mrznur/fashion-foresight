...
export function UserDashboard() {
  ...
  const orders = [
    { id: '5421', date: '2026-04-20', status: 'Delivered', total: 229, items: 2, image: 'https://images.unsplash.com/photo-1578909541713-3451b5400dbc?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=400' },
    { id: '5398', date: '2026-04-15', status: 'Delivered', total: 159, items: 1, image: 'https://images.unsplash.com/photo-1580682312385-e94d8de1cf3c?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=400' },
  ];
                ...
                <div className="p-6 space-y-4">
                  {orders.map((order) => (
                    <div key={order.id} className="flex gap-4 p-4 border border-[#e5e5e5] rounded-2xl hover:border-[#64020e]/30 hover:shadow-md transition-all duration-200">
                      <img src={order.image} alt="Order" className="w-18 h-18 object-cover rounded-xl" style={{ width: '72px', height: '72px' }} />
                      <div className="flex-1">
                        <div className="flex items-start justify-between mb-2">
                          <div>
                            <p className="text-sm font-semibold text-[#1a0508]">Order #{order.id}</p>
                            <p className="text-xs text-[#737373]">{order.date}</p>
                          </div>
                          <span className="px-2.5 py-1 bg-green-100 text-green-700 text-[10px] font-semibold rounded-full">{order.status}</span>
                        </div>
                        <p className="text-xs text-[#737373] mb-1">{order.items} {order.items === 1 ? 'item' : 'items'}</p>
                        <p className="text-base font-bold text-[#1a0508]">${order.total}</p>
                      </div>
                      <button className="btn-outline px-4 py-2 text-xs h-fit">View</button>
                    </div>
                  ))}
                      ...
                      {wishlistItems.map((item) => (
                        <div key={item.id} className="group border border-[#e5e5e5] rounded-2xl overflow-hidden hover:border-[#64020e]/30 hover:shadow-md transition-all duration-200">
                          ...
                          <div className="p-4">
                            ...
                            <p className="text-sm font-semibold text-[#1a0508] mb-3">${item.price}</p>
